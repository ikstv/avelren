...
Custom dump створюється з `--no-owner --no-acl`, копіюється на host лише після status `0`, а фактично перенесений host dump передається через stdin до `pg_restore --list` у тому самому PostgreSQL container з окремим bounded transfer timeout. Host `pg_restore` не потрібен; validation не підключається до database і завершується до `restic backup`. Після validation dump шифрується Restic і видаляється з root-only temporary directory. Host dump створюється атомарно як root-owned regular file з exact mode `0600` незалежно від umask викликача; наявний symlink, FIFO, directory або regular file відхиляється. Backup не виконує SQL mutation. Prune є окремою командою: 7 daily, 4 weekly, 3 monthly; warning — 12 GiB, hard stop нових backup-ів — 14 GiB.
...
The custom dump uses `--no-owner --no-acl` and is copied to the host only after status `0`. The actually transferred host dump is then streamed over stdin to `pg_restore --list` in the same PostgreSQL container under the dedicated bounded transfer timeout. Host `pg_restore` is not required; validation makes no database connection and completes before `restic backup`. After validation, the dump is encrypted with Restic and removed from its root-only temporary directory. The host dump is atomically created as a root-owned regular file with exact mode `0600`, independent of caller umask; an existing symlink, FIFO, directory, or regular file is rejected. Backup executes no mutating SQL. Prune is separate: 7 daily, 4 weekly, and 3 monthly snapshots; warning at 12 GiB and hard stop for new backups at 14 GiB.
